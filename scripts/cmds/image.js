...
module.exports = {
  config: {
    ...
    longDescription: 'Usage: -image <search_query>',
  },

  onStart: async function ({ api, event, args }) {
    try {
      if (args.length === 0) {
        return api.sendMessage('📷 | Follow this format:\n-image naruto uzumaki', event.threadID, event.messageID);
      }
      ...
      const images = response.data.items.slice(0, 4); // Limit to the first 4 images
